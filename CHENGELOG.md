...
| トピック | 別案 | 採用しなかった理由 |
|----------|------|--------------------|
| GUI ライブラリ | Gradio, Dash | Gradio はファイルキャッシュ制御が煩雑、Dash は記述量が多い |
| 描画 | Plotly scatter | 1 万点超で重く、matplotlib の方が軽快 |
| モデル保存 | torchscript 化 | まだ CPU/ARM 互換を優先しないため後回し |

---

## 5. **既存コードとの関係・依存箇所**

- `models/conv_vae.py` の public API (`encode`, `decode`) を前提  
- `samples/latent_vis/` の `.npy` が未生成の場合は **UI 起動時に警告**するだけで学習パイプは非依存  
- GPU / CPU どちらでも動作（`map_location` 指定）
...
```bash
# ❶ 事前に scatter データを保存
python visualize/visualize_latent_grid.py  # 自動で Z, labels を npy 出力

# ❷ Streamlit アプリを起動
streamlit run visualize/visualize_latent_interactive.py
```

ブラウザで `localhost:8501` にアクセスし、
...
> *「これは完璧すぎるものができました。全然ボヤけてる感じは気になりませんでした！」* – ユーザー
> *「おお〜！潜在空間のクラスタがくっきり！」* – テスト時の所感
...
| ID  | 内容                   | 影響            | 暫定回避                    |
| --- | -------------------- | ------------- | ----------------------- |
| #17 | latent\_dim >2 非対応   | 多次元モデルは表示不可   | PCA 射影版を実装予定            |
| #18 | scatter データ未生成時に空白表示 | 初回ユーザーが迷う     | 自動生成 or サンプル同梱を検討       |
| #19 | 画像解像度 28×28 固定       | 高解像度データではぼやける | upscale / model side 変更 |

---

## 9. **今後の TODO**

* [ ] **高次元対応**：PCA / UMAP 投影 + 2 軸スライダー
* [ ] **履歴スナップショット**：気に入った z をボタンで保存 & GIF 出力
* [ ] **デプロイ**：Streamlit Community Cloud or HuggingFace Spaces
* [ ] **モバイル UI**：スライダーをタッチフレンドリーに
* [ ] **外部画像 → z 推定**：手書き数字画像をアップし潜在空間にマッピング

---

## 10. **感想・思ったこと**

> 1 日で **「学習 → 可視化 → GUI」** まで到達できたのは大きな収穫。
> ユーザー体験が一気に上がり、研究デモの説得力も向上。
> 次は latent\_dim を増やして *disentanglement* の可視化にも挑戦したい！
